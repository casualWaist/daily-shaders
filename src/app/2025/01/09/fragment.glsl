...
float map(vec4 p) {
    vec4 spherePos = vec4(-uMouse.x * 30., -uMouse.y * 10.0, -26. + ((1. - abs(uMouse.x)) + 0.038), sin(uScroll * 0.001));
    ...
    return smin(sphere, sphere2, 7.5);
}
...
void main() {
    ...
    vec3 color = texture(uTexture, vUv - uMouse + 0.5).rgb;
    vec3 colorOff = vec3(cos(uScroll * 0.1), sin(uScroll * 0.1), 1.0 - cos(uScroll * 0.1));
    float dist = distance(vUv, uMouse);
    vec3 mixedColor = mix(color, colorOff, pow(dist, 2.) * 4.);

    gl_FragColor = vec4(mixedColor, 1.0 - col.r);
}
