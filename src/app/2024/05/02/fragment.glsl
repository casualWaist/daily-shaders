...
float map(vec4 p) {
    vec4 spherePos = vec4(0., 0., 0., sin(uTime));
    float sphere = sdHyperSphere(p + vec4(sin(uTime), spherePos.y, spherePos.z, spherePos.w), 1.);
    float sphere2 = sdHyperCube(p + spherePos, vec4(1.));
...
void main() {
    ...
    for (int i = 0; i < 80; i++) {
        ...
        float d = map(vec4(p.x, p.y, p.z, sin(uTime) * 0.2));         // current distance to the scene
