...
float makeM(vec4 p, vec4 o){
    float M = sdHyperRoundBox(vec4(p.x+o.x-3.5, p.yzw + o.yzw), vec4(2.0, 3.0, 1.0, 1.0), 0.25);
    M = sSub(sdHyperTriPrismInv(vec4(p.x+o.x-3.5,p.y+o.y-4.0,p.zw+o.zw) , vec3(2.0,1.5,1.0), 0.866025, 0.25), M, 0.25);
    M = sSub(sdHyperTriPrism(vec4(p.x+o.x-2.75,p.y+o.y+5.0,p.zw+o.zw) , vec3(2.0,1.5,1.0), 1.266025, 0.25), M, 0.25);
    M = sSub(sdHyperTriPrism(vec4(p.x+o.x-4.25,p.y+o.y+5.0,p.zw+o.zw) , vec3(2.0,1.5,1.0), 1.266025, 0.25), M, 0.25);
    return M;
}
