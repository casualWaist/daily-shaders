...
const LineGlowShaderImp = shaderMaterial({
    uTime: 0,
    uProgress: 0,
...
extend({ LineGlowShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            lineGlowShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type LineGlowShaderUniforms = {
    uTime: number,
    uProgress: number,
...
type Props = LineGlowShaderUniforms & MaterialProps

const LineGlowShader = forwardRef<LineGlowShaderUniforms, Props>(({...props}: Props, ref) => {
    const localRef = useRef<Props>(null!)
    useImperativeHandle(ref, () => localRef.current)
    useFrame((state, delta) => {
        localRef.current.uTime += delta
    })
    return <lineGlowShaderImp key={LineGlowShaderImp.key} ref={localRef} attach="material" {...props} />
})
LineGlowShader.displayName = 'LineGlowShader'
...
function Scene() {
    ...
    const faceShaderRef = useRef<LineGlowShaderUniforms>(null!)
    ...
    return <mesh ref={faceRef}>
        ...
        <LineGlowShader ref={faceShaderRef}
                       uTime={0}
                       uProgress={0}
