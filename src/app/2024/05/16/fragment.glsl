...
float sdHyperFrame( vec4 p, vec4 b, float e )
{
    ...
    vec4 q = abs(p+e)-e;
    return min(min(
    length(max(vec3(p.x,q.y,q.z),0.0))+min(max(p.x,max(q.y,max(q.z, q.w))),0.0),
    length(max(vec3(q.x,p.y,q.z),0.0))+min(max(q.x,max(p.y,max(q.z, q.w))),0.0)),
    length(max(vec3(q.x,q.y,p.z),0.0))+min(max(q.x,max(q.y,max(p.z, p.w))),0.0));
}
...
float map(vec4 p) {
    ...
    float sphere2 = sdHyperFrame(p - cubePos, vec4(1., 1., 1., sin(uScroll * 0.01)), uScroll * 0.01 + 0.1);
