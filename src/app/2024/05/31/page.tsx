...
const SimulationMaterialImp = shaderMaterial({
...
extend({ SimulationMaterialImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simulationMaterialImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimulationShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
    uCurlFreq?: number
}

type SimProps = SimulationShaderUniforms & MaterialProps

const SimulationShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simulationMaterialImp key={SimulationMaterialImp.key} ref={localRef} {...props} attach='material' />
})
SimulationShader.displayName = 'SimulationShader'

const Model24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Model24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            model24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Model24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Model24ShaderUniforms & MaterialProps

const Model24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <model24ShaderImp key={Model24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Model24Shader.displayName = 'Model24Shader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: 'purple'}}>
        <Scene />
        <OrbitControls />
...
function Scene() {
    ...
    const shader = useRef<Model24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimulationShaderUniforms & ShaderMaterial>(null!)
    const testMat = useRef<THREE.MeshBasicMaterial>(null!)
    ...
    useFrame((state) => {
        ...
        testMat.current.map = target.texture
    })
    ...
    return <>
        {createPortal(
            <mesh>
                ...
                <SimulationShader ref={simShader} />
            </mesh>,
            simScene
        )}
        <group position={[0, -1, 0]} rotation={[Math.PI * 0.5, 0, 0]}>
            <points ref={points} scale={3}>
                <bufferGeometry attach="geometry" {...tGeo.nodes.Text.geometry} />
                <Model24Shader ref={shader}
                               uPositions={texture}
                               depthWrite={false}
                               depthTest={false}
                               transparent
        ...
        <mesh rotation={[0, 0, 0]}>
            <planeGeometry args={[2, 2]}/>
            <meshBasicMaterial ref={testMat} map={target.texture}/>
        </mesh>
    </>
}
