...
const EdgeShineShaderImp = shaderMaterial({
    uTime: 0,
    uProgress: 0,
...
extend({ EdgeShineShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            edgeShineShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type EdgeShineShaderUniforms = {
    uTime: number,
    uProgress: number,
...
type Props = EdgeShineShaderUniforms & MaterialProps

const EdgeShineShader = forwardRef<EdgeShineShaderUniforms, Props>(({...props}: Props, ref) => {
    const localRef = useRef<Props>(null!)
    useImperativeHandle(ref, () => localRef.current)
    useFrame((state, delta) => {
        localRef.current.uTime += delta
    })
    return <edgeShineShaderImp key={EdgeShineShaderImp.key} ref={localRef} attach="material" {...props} />
})
EdgeShineShader.displayName = 'EdgeShineShader'
...
function Scene() {
    ...
    const faceShaderRef = useRef<EdgeShineShaderUniforms>(null!)
    ...
    return <mesh ref={faceRef}>
        ...
        <EdgeShineShader ref={faceShaderRef}
                       uTime={0}
                       uProgress={0}
