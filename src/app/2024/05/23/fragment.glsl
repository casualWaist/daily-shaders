...
float makeTwo(vec4 p){
    float offset = 2.5;
    float base = sdHyperRoundBox(p + vec4(offset,-2.,0.,0.), vec4(2.5,2.,1.,1.), 1.0);
    float box = sdHyperRoundBox(p + vec4(offset,2.5,0.,0.), vec4(2.5,1.,1.,1.), 0.25);
    base = sUnion(base, box, 0.3);
    float centerCut = sdHyperRoundBox(p + vec4(offset,-0.35,0.,0.), vec4(1.,2.5,3.,1.), 0.25);
    base = sSub(centerCut, base, 0.3);
    float centerHalf = sdHyperRoundBox(p + vec4(offset,0.5,0.,0.), vec4(3.5,1.5,3.,1.), 0.25);
    base = sSub(centerHalf, base, 0.3);
    float c = cos(0.75);
    float s = sin(0.75);
    mat2  m = mat2(c,-s,s,c);
    float di = sdHyperRoundBox(vec4(m*(p.xy+vec2(offset,0.5)), p.zw + vec2(0.,0.)), vec4(2.75,0.5,1.,1.), 0.25);
...
float makeFour(vec4 p){
    float offset = 2.5;
    float base = sdHyperRoundBox(p + vec4(-offset-2.,-0.25,0.,0.), vec4(1.,3.75,1.,1.), 0.25);
    float cross = sdHyperRoundBox(p + vec4(-offset-1.,.5,0.,0.), vec4(3.5,0.5,1.,1.), 0.25);
    base = sUnion(base, cross, 0.3);
    float c = cos(0.85);
    float s = sin(0.85);
    mat2  m = mat2(c,-s,s,c);
    float di = sdHyperRoundBox(vec4(m*(p.xy+vec2(-offset+0.5,-1.5)), p.zw + vec2(0.,0.)), vec4(2.75,0.5,1.,1.), 0.25);
    base = sUnion(base, di, 0.3);
...
float makeRing(vec4 p) {
    ...
    mat2  m = mat2(s,c,-c,s);
    vec2 q = vec2(m*p.yz);
    vec4 fp = vec4(p.x+8.25*s, q.x+8.25*c, q.y, p.w);
    float ring = sdHyperRoundedCylinder(vec4(p.x,q.x,q.y,p.w), 4.2, 1., 0.0625);
    ring = sSub(sdHyperRoundedCylinder(vec4(p.x,q.x,q.y,p.w), 4.12, 1.5, 3.0), ring, 0.3);
    ring = sUnion(sdHyperSphere(fp, 0.5), ring, 0.01);
    c = cos(uTime + 1.);
    s = sin(uTime + 1.);
    m = mat2(s,c,-c,s);
    q = vec2(m*p.xz);
    fp = vec4(q.x+8.25*c, p.y+8.25*s, q.y, p.w);
    float ring2 = sdHyperRoundedCylinder(vec4(q.x,p.y,q.y,p.w), 4.2, 1., 0.0625);
    ring2 = sSub(sdHyperRoundedCylinder(vec4(q.x,p.y,q.y,p.w), 4.12, 1.5, 3.0), ring2, 0.3);
    ...
    fp = vec4(q.x+8.25*c, p.y+8.25*s, q.y+o.y, p.w);
    float ring3 = sdHyperRoundedCylinder(vec4(q.x,p.y,q.y+o.y,p.w), 4.2, 1., 0.0625);
    ring3 = sSub(sdHyperRoundedCylinder(vec4(q.x,p.y,q.y+o.y,p.w), 4.12, 1.5, 3.0), ring3, 0.3);
