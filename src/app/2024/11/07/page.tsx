...
import {Float, Html, OrbitControls, shaderMaterial, Svg, useFBO, useGLTF, useTexture} from '@react-three/drei'
...
const SimVirusShaderImp = shaderMaterial({
...
extend({ SimVirusShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simVirusShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimVirusShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
    uTexture?: THREE.Texture
}

type SimProps = SimVirusShaderUniforms & MaterialProps

const SimVirusShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simVirusShaderImp key={SimVirusShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimVirusShader.displayName = 'SimVirusShader'

const VirusCityShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ VirusCityShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            virusCityShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type VirusCityShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = VirusCityShaderUniforms & MaterialProps

const VirusCityShader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <virusCityShaderImp key={VirusCityShaderImp.key} ref={localRef} {...props} attach='material' />
})
VirusCityShader.displayName = 'VirusCityShader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#51ff5f'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
                  center transform as="h1" className={caveat.className} scale={0.25}>
                <div style={{ transform: 'scale(4)', textAlign: 'center', color: '#78008d' }}>
                    Not even the 4th
                </div>
            </Html>
        </Float>
...
function Scene() {
    ...
    const shader = useRef<VirusCityShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimVirusShaderUniforms & ShaderMaterial>(null!)
    const points = useRef<THREE.Points>(null!)

    const tGeo = useGLTF('/mapVirus.glb') as GLTFResult
