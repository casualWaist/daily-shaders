...
const CicadaSimShaderImp = shaderMaterial({
...
declare global {
    namespace JSX {
        interface IntrinsicElements {
            cicadaSimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type CicadaSimShaderUniforms = {
    uTime?: number
...
const CicadaSimShader = forwardRef(({ ...props }: SimProps, ref) => {
...
const CicadaShaderImp = shaderMaterial({
    uTime: 0,
    uProgress: 0,
    uPositions: new THREE.DataTexture(),
...
declare global {
    namespace JSX {
        interface IntrinsicElements {
            cicadaShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type CicadaShaderUniforms = {
    uTime?: number
    uProgress?: number
    uPositions?: THREE.DataTexture
...
type Props = CicadaShaderUniforms & MaterialProps

const CicadaShader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    useFrame((_, delta) => {
        localRef.current.uTime += delta
        if (localRef.current.uProgress < 0.70) localRef.current.uProgress += 0.002
        else if (localRef.current.uProgress < 0.90) localRef.current.uProgress += 0.001
...
export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#000000'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
                  center transform as="h1" className={caveat.className} scale={0.25}>
                <div style={{transform: 'scale(4)', textAlign: 'center', color: '#9b3a3a'}}>
                    A Mystery?
                </div>
            </Html>
        </Float>
...
function Scene() {
    ...
    const shader = useRef<CicadaShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<CicadaSimShaderUniforms & ShaderMaterial>(null!)
