...
const VolumeCycleShaderImp = shaderMaterial({
    uTime: 0,
    ...
    uNoiseScale: new THREE.Vector4(1.75, 1.75, 1.0, 0.25),
...
extend({ VolumeCycleShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            volumeCycleShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}
...
const VolumeCycleShader = forwardRef(({...props}: Props, ref) => {
    ...
    return <volumeCycleShaderImp key={VolumeCycleShaderImp.key} ref={localRef} attach="material" {...props} />
})
VolumeCycleShader.displayName = 'VolumeCycleShader'
    ...
    return <Canvas style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: "#000"}}>
        ...
        <Float>
            <Html position={[0, -1.5, 1]}
                ...
                scale={0.25}>
                <div style={{transform: 'scale(4)', color: "#181615"}}>
                    Because 3D
                </div>
            </Html>
        </Float>
...
function Scene() {
    ...
    return <>
        <mesh ref={meshRef}>
            <boxGeometry args={[view.width * 0.25, view.width * 0.25, view.width * 0.25]}/>
            <VolumeCycleShader ref={shaderRef}
                               uTime={0}
                               uTexture={texture}
                               transparent
                               depthTest={false}
        ...
        <mesh ref={boxRef} position={[0, -1.5, 0]}>
            <boxGeometry args={[view.width * 0.25, view.width * 0.0625, view.width * 0.25]}/>
            <meshStandardMaterial color={"#423907"} />
        </mesh>
        <directionalLight intensity={20} position={[-1, 1, 0.2]}/>
        <directionalLight intensity={10} position={[1, 1, 0.4]}/>
    </>
}
