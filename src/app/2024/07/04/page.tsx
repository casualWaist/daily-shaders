...
const WorldSimShaderImp = shaderMaterial({
...
extend({ WorldSimShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            worldSimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type WorldSimShaderUniforms = {
    uTime?: number
...
type SimProps = WorldSimShaderUniforms & MaterialProps

const WorldSimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <worldSimShaderImp key={WorldSimShaderImp.key} ref={localRef} {...props} attach='material' />
})
WorldSimShader.displayName = 'WorldSimShader'

const WorldMapDotsShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ WorldMapDotsShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            worldMapDotsShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type WorldMapDotsShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = WorldMapDotsShaderUniforms & MaterialProps

const WorldMapDotsShader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <worldMapDotsShaderImp key={WorldMapDotsShaderImp.key} ref={localRef} {...props} attach='material' />
})
WorldMapDotsShader.displayName = 'WorldMapDotsShader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#796e9f'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
                  center transform as="h1" className={caveat.className} scale={0.25}>
                <div style={{transform: 'scale(4)', textAlign: 'center', color: '#28334b'}}>
                    News You Need<br/>Nooowww
                </div>
            </Html>
        </Float>
    </Canvas>
...
function Scene() {
    ...
    const shader = useRef<WorldMapDotsShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<WorldSimShaderUniforms & ShaderMaterial>(null!)
    ...
    useFrame((state) => {
        ...
        //points.current.rotation.z = state.clock.elapsedTime % Math.PI * 2
    })
