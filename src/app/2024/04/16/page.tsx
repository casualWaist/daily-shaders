...
const ShagEyeShaderImp = shaderMaterial({
    uTime: 0,
...
extend({ ShagEyeShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            shagEyeShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type ShagEyeShaderUniforms = {
    uTime: number
...
type Props = ShagEyeShaderUniforms & MaterialProps

const ShagEyeShader = forwardRef<ShagEyeShaderUniforms, Props>(({...props}: Props, ref) => {
    ...
    return <shagEyeShaderImp key={ShagEyeShaderImp.key} ref={localRef} attach="material" {...props} />
})
ShagEyeShader.displayName = 'ShagEyeShader'
...
function Scene({props}: {props?: JSX.IntrinsicElements['mesh']}) {
    ...
    const shaderRef = useRef<ShagEyeShaderUniforms>(null!)
