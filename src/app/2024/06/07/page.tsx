...
const SimTimShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        vec3 curlPos = texture(uTexture, vUv).rgb;
        pos = pos + curl(pos + uTime) * mod(uTime, 0.05);
        //curlPos += curl(-curlPos * uCurlFreq * 16.0) * 0.03125;
        curlPos += curl(pos * uCurlFreq * 16.0) + mod(sin(uTime) * 0.125 + 0.125, 0.5);
        gl_FragColor = vec4(pos, noise(curlPos * sin(uTime) * 0.01 * noise(pos + uTime * 17.0)) * 2.125 + 1.125);
    }`
)

extend({ SimTimShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simTimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimTimShaderUniforms = {
    uTime?: number
...
type SimProps = SimTimShaderUniforms & MaterialProps

const SimTimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simTimShaderImp key={SimTimShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimTimShader.displayName = 'SimTimShader'

const Shutter24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Shutter24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            shutter24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Shutter24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Shutter24ShaderUniforms & MaterialProps

const Shutter24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <shutter24ShaderImp key={Shutter24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Shutter24Shader.displayName = 'Shutter24Shader'
...
function Scene() {
    ...
    const shader = useRef<Shutter24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimTimShaderUniforms & ShaderMaterial>(null!)
