...
const SimCityShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        gl_FragColor = vec4(min(pos, curlPos), 1.0);
    }`
)

extend({ SimCityShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simCityShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimCityShaderUniforms = {
    uTime?: number
...
type SimProps = SimCityShaderUniforms & MaterialProps

const SimCityShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simCityShaderImp key={SimCityShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimCityShader.displayName = 'SimCityShader'

const Rainbow24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Rainbow24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            rainbow24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Rainbow24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Rainbow24ShaderUniforms & MaterialProps

const Rainbow24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <rainbow24ShaderImp key={Rainbow24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Rainbow24Shader.displayName = 'Rainbow24Shader'
...
function Scene() {
    ...
    const shader = useRef<Rainbow24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimCityShaderUniforms & ShaderMaterial>(null!)
