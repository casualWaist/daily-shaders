...
const SimSimsShaderImp = shaderMaterial({
...
extend({ SimSimsShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simSimsShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimSimsShaderUniforms = {
    uTime?: number
...
type SimProps = SimSimsShaderUniforms & MaterialProps

const SimSimsShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simSimsShaderImp key={SimSimsShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimSimsShader.displayName = 'SimSimsShader'

const Dragon24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Dragon24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            dragon24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Dragon24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Dragon24ShaderUniforms & MaterialProps

const Dragon24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <dragon24ShaderImp key={Dragon24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Dragon24Shader.displayName = 'Dragon24Shader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#d9ff51'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
...
function Scene() {
    ...
    const shader = useRef<Dragon24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimSimsShaderUniforms & ShaderMaterial>(null!)
