...
const HorseSimShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        gl_FragColor = vec4(mix(pos, curlPos, sin(t) * 0.5 + 0.5), 1.0);
    }`
)

extend({ HorseSimShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            horseSimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type HorseSimShaderUniforms = {
    uTime?: number
...
type SimProps = HorseSimShaderUniforms & MaterialProps

const HorseSimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <horseSimShaderImp key={HorseSimShaderImp.key} ref={localRef} {...props} attach='material' />
})
HorseSimShader.displayName = 'HorseSimShader'

const Horsee24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Horsee24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            horsee24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Horsee24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Horsee24ShaderUniforms & MaterialProps

const Horsee24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <horsee24ShaderImp key={Horsee24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Horsee24Shader.displayName = 'Horsee24Shader'
...
function Scene() {
    ...
    const shader = useRef<Horsee24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<HorseSimShaderUniforms & ShaderMaterial>(null!)
    ...
    return <>
        ...
        <group position={[0, -1.5 * (view.width / 11.0), 0]} rotation={[Math.PI * 0.5, 0, 0]}>
            <points ref={points} scale={ 5 * (view.width / 11.0) }>
                ...
                <Horsee24Shader ref={shader}
                               uPositions={texture}
                               depthWrite={false}
                               transparent
                               blending={THREE.NormalBlending}
                               uColor={new THREE.Color(0.26, 0.2333, 0.904)}/>
            </points>
        </group>
    </>
}
