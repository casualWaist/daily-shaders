...
const TheSimsShaderImp = shaderMaterial({
    ...
    void main() {
        float t = uTime * 0.5;
        vec3 pos = texture(uPositions, mod(vUv, sin(t))).rgb;
        vec3 curlPos = texture(uTexture, vUv).rgb;
        curlPos.y = mix(pos.y, curlPos.y, uTime * 0.1);
        pos = pos - curl(pos + t) * sin(uTime);
        curlPos = curl(curlPos * uCurlFreq + t) * 0.1;
        gl_FragColor = vec4(mix(pos, curlPos, sin(t)), 1.0);
    }`
)

extend({ TheSimsShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            theSimsShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type TheSimsShaderUniforms = {
    uTime?: number
...
type SimProps = TheSimsShaderUniforms & MaterialProps

const TheSimsShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <theSimsShaderImp key={TheSimsShaderImp.key} ref={localRef} {...props} attach='material' />
})
TheSimsShader.displayName = 'TheSimsShader'

const Tiger24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Tiger24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            tiger24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Tiger24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Tiger24ShaderUniforms & MaterialProps

const Tiger24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <tiger24ShaderImp key={Tiger24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Tiger24Shader.displayName = 'Tiger24Shader'
...
function Scene() {
    ...
    const shader = useRef<Tiger24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<TheSimsShaderUniforms & ShaderMaterial>(null!)
