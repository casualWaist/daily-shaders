...
const LazySimShaderImp = shaderMaterial({
...
extend({ LazySimShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            lazySimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type LazySimShaderUniforms = {
    uTime?: number
...
type SimProps = LazySimShaderUniforms & MaterialProps

const LazySimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <lazySimShaderImp key={LazySimShaderImp.key} ref={localRef} {...props} attach='material' />
})
LazySimShader.displayName = 'LazySimShader'

const Cray24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Cray24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            cray24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Cray24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Cray24ShaderUniforms & MaterialProps

const Cray24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <cray24ShaderImp key={Cray24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Cray24Shader.displayName = 'Cray24Shader'
...
function Scene() {
    ...
    const shader = useRef<Cray24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<LazySimShaderUniforms & ShaderMaterial>(null!)
