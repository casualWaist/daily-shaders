...
void main() {
    ...
    float red = normalize(dot(vUv, vPoints));
    float green = normalize(dot(vUv, vPoint));
    float blue = normalize(dot(vUv, vPoint));

    vec4 color = vec4(ring * red, 0.0, 0.0, ring);
    //gl_FragColor = color;
    gl_FragColor = vec4(vec3(curl(uColor + ring) * vec3(red, green, blue)), 1.0);
