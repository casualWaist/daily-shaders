...
const SimTinShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        gl_FragColor = vec4(mix(pos, curlPos, 1.0), 1.0);
    }`
)

extend({ SimTinShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simTinShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimTinShaderUniforms = {
    uTime?: number
...
type SimProps = SimTinShaderUniforms & MaterialProps

const SimTinShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simTinShaderImp key={SimTinShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimTinShader.displayName = 'SimTinShader'

const Timm24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Timm24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            timm24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Timm24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Timm24ShaderUniforms & MaterialProps

const Timm24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <timm24ShaderImp key={Timm24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Timm24Shader.displayName = 'Timm24Shader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#6e9f83'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
                  center transform as="h1" className={caveat.className} scale={0.25}>
                <a href="https://en.wikipedia.org/wiki/Georges_Seurat"
                   target="_blank"
                   style={{ transform: 'scale(4)', fontSize: 'xxx-large', textAlign: 'center', color: '#643b72' }}>
                    Georges Seurat
                </a>
            </Html>
        </Float>
...
function Scene() {
    ...
    const shader = useRef<Timm24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimTinShaderUniforms & ShaderMaterial>(null!)
    ...
    return <>
        ...
        <group position={[0, -1.5 * (view.width / 11.0), 0]} rotation={[Math.PI * 0.5, 0, 0]}>
            <points ref={points} scale={ 5 * (view.width / 11.0) }>
                ...
                <Timm24Shader ref={shader}
                               uPositions={texture}
                               depthWrite={false}
                               transparent
                               blending={THREE.NormalBlending}
                               uColor={new THREE.Color(0.96, 0.8333, 0.604)}/>
            </points>
        </group>
    </>
}
