...
const DimSimShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        gl_FragColor = vec4(min(pos, curlPos), mix(1.0, noise(curlPos + t), uTime * 0.1 - uTime * 0.09));
    }`
)

extend({ DimSimShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            dimSimShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type DimSimShaderUniforms = {
    uTime?: number
...
type SimProps = DimSimShaderUniforms & MaterialProps

const DimSimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <dimSimShaderImp key={DimSimShaderImp.key} ref={localRef} {...props} attach='material' />
})
DimSimShader.displayName = 'DimSimShader'

const Star24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Star24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            star24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Star24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Star24ShaderUniforms & MaterialProps

const Star24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <star24ShaderImp key={Star24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Star24Shader.displayName = 'Star24Shader'
...
function Scene() {
    ...
    const shader = useRef<Star24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<DimSimShaderUniforms & ShaderMaterial>(null!)
    ...
    useFrame((state) => {
        ...
        camera.position.z += state.clock.elapsedTime * 0.0001
