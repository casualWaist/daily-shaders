...
const SimPsonsShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        vec2 aUv = uv - 0.5;
        vUv = vec2(aUv.x * cosAngle - aUv.y * sinAngle, aUv.x * sinAngle + aUv.y * cosAngle);
...
extend({ SimPsonsShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            simPsonsShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimPsonsShaderUniforms = {
    uTime?: number
...
type SimProps = SimPsonsShaderUniforms & MaterialProps

const SimPsonsShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simPsonsShaderImp key={SimPsonsShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimPsonsShader.displayName = 'SimPsonsShader'

const Rat24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Rat24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            rat24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Rat24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Rat24ShaderUniforms & MaterialProps

const Rat24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <rat24ShaderImp key={Rat24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Rat24Shader.displayName = 'Rat24Shader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#796c00'}}>
        <Scene />
        <OrbitControls />
        <Float>
            <Html position={[0, -2, 1]}
                  center transform as="h1" className={caveat.className} scale={0.25}>
                <div style={{ transform: 'scale(4)', textAlign: 'center', color: '#e09173' }}>
                    Dust in the...<br/>Whoa
                </div>
            </Html>
        </Float>
...
function Scene() {
    ...
    const shader = useRef<Rat24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimPsonsShaderUniforms & ShaderMaterial>(null!)
