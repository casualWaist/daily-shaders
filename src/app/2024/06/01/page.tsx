...
const SimShaderImp = shaderMaterial({
    ...
    void main() {
        ...
        //curlPos += curl(curlPos * uCurlFreq * 4.0) * 0.0125;
        //curlPos += curl(pos * uCurlFreq * 8.0) * 0.03125;
...
declare global {
    namespace JSX {
        interface IntrinsicElements {
            simShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type SimShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
    uCurlFreq?: number
}

type SimProps = SimShaderUniforms & MaterialProps

const SimShader = forwardRef(({ ...props }: SimProps, ref) => {
    ...
    return <simShaderImp key={SimShaderImp.key} ref={localRef} {...props} attach='material' />
})
SimShader.displayName = 'SimShader'

const Wave24ShaderImp = shaderMaterial({
    uTime: 0,
    uPositions: new THREE.DataTexture(),
...
extend({ Wave24ShaderImp })

declare global {
    namespace JSX {
        interface IntrinsicElements {
            wave24ShaderImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}

export type Wave24ShaderUniforms = {
    uTime?: number
    uPositions?: THREE.DataTexture
...
type Props = Wave24ShaderUniforms & MaterialProps

const Wave24Shader = forwardRef(({ ...props }: Props, ref) => {
    const localRef = useRef<THREE.ShaderMaterial & {
    ...
    return <wave24ShaderImp key={Wave24ShaderImp.key} ref={localRef} {...props} attach='material' />
})
Wave24Shader.displayName = 'Wave24Shader'

export default function Page() {
    return <Canvas  style={{position: "absolute", top: "0", zIndex: "-1", backgroundColor: '#ead152'}}>
        <Scene />
        <OrbitControls />
...
function Scene() {
    ...
    const shader = useRef<Wave24ShaderUniforms & ShaderMaterial>(null!)
    const simShader = useRef<SimShaderUniforms & ShaderMaterial>(null!)
