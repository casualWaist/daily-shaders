...
const MarchFireImp = shaderMaterial({
    uTime: 0,
    uTexture: new THREE.Texture(),
    uColor: new THREE.Color(0.9375, 0.9375, 0.9375),
    uInvModelMatrix: new THREE.Matrix4(),
    uScale: new THREE.Vector3( 0.25, 0.25, 0.25 ),
...
declare global {
    namespace JSX {
        interface IntrinsicElements {
            marchFireImp: MaterialNode<any, typeof THREE.MeshStandardMaterial>
        }
    }
}
...
const MarchFire = forwardRef(({...props}: Props, ref) => {
    ...
    return <marchFireImp key={MarchFireImp.key} ref={localRef} attach="material" {...props} />
})
MarchFire.displayName = 'MarchFire'
        ...
        <Float>
            <Html position={[0, -1.5, 1]}
                ...
                <div style={{transform: 'scale(4)'}}>
                    aaand it doesn't work.<br/>TODO: Fix this.
                </div>
            </Html>
        </Float>
        <OrbitControls />
...
function Scene() {
    ...
    useFrame(() => {
        ...
        shaderRef.current.shaderImp.uInvModelMatrix.invert(meshRef.current.matrix)
    })
    return <mesh ref={meshRef}>
        <boxGeometry args={[view.width * 0.25, view.height * 0.25, view.width * 0.25]}/>
        <MarchFire ref={shaderRef} uTime={0} uTexture={texture} uColor={new THREE.Color(1, 1, 1)} />
    </mesh>
}
