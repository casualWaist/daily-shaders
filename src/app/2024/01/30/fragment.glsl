...
float map(vec3 p) {
    vec3 spherePos = vec3(time * 0.01, 0., 0.);
    vec3 spherePos2 = vec3(0., time * 0.1, 0.);
    vec3 spherePos3 = vec3(time * 0.1, 0., 0.);
    float sphere = sdSphere(p - spherePos, 0.5);
    float sphere2 = sdSphere(p + spherePos2, 0.5);
    vec3 q;
    float angle = time;
    q.xz = mat2(cos(angle),-sin(angle),sin(angle),cos(angle)) * p.yz;
    if (p.z < 0.) q.z *= -1. + spherePos3.x;
    float pill = sdVerticalCapsule(q - spherePos3, 0.25, 0.25);
