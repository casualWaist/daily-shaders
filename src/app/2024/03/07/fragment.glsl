...
void main(){
    ...
    uv = mod(uv, 1.0);
    float bandsY = 1.0 - step(0.5, uv.x);
    float bandsX = 1.0 - step(0.5, uv.y);
    float hatch = smoothstep(color.r, bandsY, intensity);

    gl_FragColor = vec4(mix(uColor, vec3(0.2, 0.4, 0.7), hatch), 1.0);
