...
import earthFragment from '../22/fragment.glsl'
import earthVertex from '../22/vertex.glsl'
...
const caveat = Caveat({
 ...
 }, earthVertex, earthFragment, (imp) => { if (imp) {
     //imp.wireframe = true
 } })
 ...
 type ESProps = EarthShaderUniforms & MaterialProps

 const EarthShader = forwardRef<EarthShaderUniforms, ESProps>(({...props}: ESProps, ref) => {
     const localRef = useRef<ESProps>(null!)
...
type Props = AtmosphereShaderUniforms & MaterialProps

const AtmosphereShader = forwardRef<AtmosphereShaderUniforms, Props>(({...props}: Props, ref) => {
...
function Scene({props}: {props?: JSX.IntrinsicElements['mesh']}) {
    ...
    const shader = useRef<AtmosphereShaderUniforms>(null!)
